...
public class AiChatModelFactory
{
    ...
    {
        return createChatModel(credentialProvider.resolveCredential(), temperature);
    }

    public ChatModel createChatModel(AiModelCredential cred, double temperature)
    {
        if (cred == null)
        {
            throw new IllegalStateException("AiModelCredential is null");
        }
    ...
    {
        return createStreamingChatModel(credentialProvider.resolveCredential(), temperature);
    }

    public StreamingChatModel createStreamingChatModel(AiModelCredential cred, double temperature)
    {
        if (cred == null)
        {
            throw new IllegalStateException("AiModelCredential is null");
